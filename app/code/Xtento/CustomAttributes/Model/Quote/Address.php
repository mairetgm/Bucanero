 ...
 * File:          app/code/Xtento/CustomAttributes/Model/Quote/Address.php
 * Copyright:     Copyright (c) XTENTO GmbH & Co. KG <info@xtento.com> / All rights reserved.
 */

namespace Xtento\CustomAttributes\Model\Quote;
...
class Address
{
    ...
    public function extensionAttributes($address) {
        ...
        $customerExtensionProcessed = $extensionAttributes->getCustomer();
        $customerAddressExtensionProcessed = $extensionAttributes->getCustomerAddress();
        $orderExtensionProcessed = $extensionAttributes->getOrderField();

        if ($customerExtensionProcessed instanceof AttributeData) {
            foreach ($customerExtensionProcessed->getValue() as $key => $value) {
                $address->setData($key, $value);
            }
        }

        if ($customerAddressExtensionProcessed instanceof AttributeData) {
            foreach ($customerAddressExtensionProcessed->getValue() as $key => $value) {
                $address->setData($key, $value);
            }
        }

        if ($orderExtensionProcessed instanceof AttributeData) {
            foreach ($orderExtensionProcessed->getValue() as $key => $value) {
                $address->setData($key, $value);
            }
        }
