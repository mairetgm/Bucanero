 ...
 * File:          app/code/Xtento/CustomAttributes/Model/ResourceModel/Collection/AbstractCollection.php
...
abstract class AbstractCollection extends MagentoAbstractCollection
{
    ...
    public function performAfterLoad($tableName, $linkField)
    {
        ...
        if (!empty($linkedIds)) {
            ...
            $select = $connection->select()->from(['xtento_attributes_field_store' => $this->getTable($tableName)])
                ->where('xtento_attributes_field_store.' . $linkField . ' IN (?)', $linkedIds);
