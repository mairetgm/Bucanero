...
class OrderCommentSender extends \Magento\Sales\Model\Order\Email\Sender\OrderCommentSender
{
    ...
    protected function _customCheckAndSend($order, $notify)
    {
        ...
        if ($amastyStatus !== null && !$notify) {
            $notify = $amastyStatus->getNotifyByEmail();
        }

        $this->_coreRegistry->register('amorderstatus_store_id', $order->getStoreId(), true);
        $this->_coreRegistry->register('amorderstatus_state', $order->getState(), true);
    ...
    protected function _prepareTemplate($order)
    {
        ...
        if ($ourStatus) {
            ...
            $ourTemplateCollection =
                $this->_objectManager->get(\Amasty\OrderStatus\Model\ResourceModel\Template\Collection::class);
