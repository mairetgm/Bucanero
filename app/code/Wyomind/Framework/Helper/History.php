    ...
    public function createVersionHistoryTable($installer, $entityTableName)
    {
        ...
        if (!$installer->tableExists($tableName)) {
            $table = $installer->getConnection()
                ...
                ->addIndex(
                    $installer->getIdxName($tableName, ['id']), ['id']
                )
                ->addIndex(
                    $installer->getIdxName($tableName, ['entity_id']), ['entity_id']
                )
                ->addIndex(
                    $installer->getIdxName($tableName, ['version_id']), ['version_id']
                )
                ->addIndex(
                    $installer->getIdxName($tableName, ['username']), ['username']
                )
    ...
    public function createActionHistoryTable($installer, $entityTableName)
    {
        ...
        if (!$installer->tableExists($tableName)) {
            $table = $installer->getConnection()
                ...
                ->addIndex(
                    $installer->getIdxName($tableName, ['action_id']), ['action_id']
                )
                ->addIndex(
                    $installer->getIdxName($tableName, ['entity_id']), ['entity_id']
                )
                ...
                ->addIndex(
                    $installer->getIdxName($tableName, ['action_type']), ['action_type']
                )
                ->addIndex(
                    $installer->getIdxName($tableName, ['origin']), ['origin']
                )
                ->addIndex(
                    $installer->getIdxName($tableName, ['username']), ['username']
                )
                ->addIndex(
                    $installer->getIdxName($tableName, ['result']), ['result']
                )
