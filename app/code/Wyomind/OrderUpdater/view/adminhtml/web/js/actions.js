...
//                    this.sortable();
                    document.observe('click', function (elt) {
                        ...
                        // }
                        // if (elt.findElement(".icon.condition-trash")) {
                        //     var row = elt.findElement(".icon.condition-trash").up(".conditions-row");
                        //     this.row.delete(row, this)
                        // }
                        // if (elt.findElement(".icon.action-trash")) {
                        //     var row = elt.findElement(".icon.action-trash").up(".actions-row");
                        //     this.row.delete(row, this)
                        // }
                        // if (elt.findElement(".icon.rule-trash")) {
                        //     var row = elt.findElement(".icon.rule-trash").up(".rule-row");
                        //     this.row.delete(row, this)
                        // }
                        //
                        // if (elt.findElement(".rules-row .code")) {
                        //     var row = elt.findElement(".actions-row .code").up("li");
                        ...
                        // }
//                         if (elt.findElement(".rule-row .link")) {
//                             var row = elt.findElement(".rule-row .link").up(".rule-row");
//                             this.row.activate(row, this)
//                         }
//                         if (elt.findElement(".icon.condition-add")) {
//                             require(['wyomind_OrderUpdater_profilerenderer'], function (profilerenderer) { profilerenderer.renderConditions('ou-condition-template', elt.findElement(".icon.condition-add").ancestors()[1], 'after', true); });
//                             //this.row.addCondition(this, elt.findElement(".icon.condition-add").ancestors()[1])
//                         }
//                         if (elt.findElement(".icon.action-add")) {
//                             require(['wyomind_OrderUpdater_profilerenderer'], function (profilerenderer) { profilerenderer.renderActions('ou-action-template', elt.findElement(".icon.action-add").ancestors()[1], 'after', true); });
// //                            this.row.addAction(this, elt.findElement(".icon.action-add").ancestors()[1])
//                         }
//                         if (elt.findElement(".icon.rule-add")) {
//                             require(['wyomind_OrderUpdater_profilerenderer'], function (profilerenderer) { profilerenderer.renderRules('','ou-rule-template','ou-condition-template', 'ou-action-template', elt.findElement(".icon.rule-add").ancestors()[3], 'after', '.conditions-title', true); });
//                             //this.row.addRule(this, elt.findElement(".icon.rule-add").ancestors()[3])
//                         }
                    ...
                    //     if (elt.findElement('.condition')) {
                    //         var condition = elt.findElement(".condition");
                    //         var row = condition.up(".conditions-row");
                    //         this.row.updateCondition(row, condition);
                    //     }
                    //     if (elt.findElement('.condition-attribute')) {
                    //         var operand = elt.findElement(".condition-attribute");
                    //         var row = operand.up(".conditions-row");
                    //         this.row.updateConditionOperand(row, operand);
                    //     }
                    ...
                    //     }
                    //     if (elt.findElement(".actions-row .action")) {
                    //         var row = elt.findElement(".actions-row .action").up("li");
                    //         require(['wyomind_OrderUpdater_profilerenderer'], function (profilerenderer) { profilerenderer.renderActionOptions('','ou-rule-template','ou-condition-template', 'ou-action-template', elt.findElement(".icon.rule-add").ancestors()[3], 'after', '.conditions-title', true); });
                    //
                    //         this.row.save(row, this)
                    //     }
                    //     if (elt.findElement(".actions-row .source")) {
                    //         var row = elt.findElement(".actions-row .source").up("li");
                    //
                    ...
                    //         this.row.save(row, this)
                    //     }
                    //     if (elt.findElement(".actions-row .default.value")) {
                    //         var row = elt.findElement(".actions-row .default").up("li");
                    //         this.row.save(row, this);
                    //     }
            ...
            updateSource: function () {
                var rules = $("rules").getValue().evalJSON();
                ...
                $$("#rules-area .source").each(function (src, i) {
                    src.update(options.join(""));
                    src.select("OPTION")[0].selected = true;
                    src.select("OPTION").each(function (option, x) {
                        if (typeof rules[i].source != "undefined" && rules[i].source == option.innerText) {
                            src.select("OPTION")[x].selected = true;
                        } else if (rules[i].index == option.value) {
                            src.select("OPTION")[x].selected = true;
                        }
                    })
            ...
            save: function () {
                ...
                $$(".aggregate").each(function (aggregate) {
                    if (aggregate.getValue() != "") {
                        rules.push(aggregate.getValue().evalJSON());
                    }
                });
                $("rules").setValue(Object.toJSON(rules));
            }
            ,
            row: {
                    ...
                    jQuery('.color').ColorPicker({
                            ...
                            onSubmit:
                                function (hsb, hex, rgb, el) {
                                    ...
                                    require(["wyomind_OrderUpdater_actions"], function (actions) {
                                        actions.row.save(jQuery(el).parents("LI")[0], actions);
                                    })
                                }
                ...
                ,
                addCondition: function (parent, elt) {
                    //console.log(parent);
                    console.log(elt);
                    elt.insert({after: conditionTemplate});
                ...
                ,
                addAction: function (parent, elt) {
                    //console.log(parent);
                    console.log(elt);
                    elt.insert({after: actionTemplate});
                ...
                ,
                /**
                 * Update a condition row (enable, disable, ...)
                 * @param {type} id
                ...
                //         jQuery(parent).find(".condition-attribute, .value-attribute").each(function() {jQuery(this).prop('disabled', false)});
                //     } else {
                //         jQuery(parent).find(".condition-attribute, .value-attribute").each(function() {jQuery(this).prop('disabled', true)});
                //     }
                // }
                // ,
                /**
                ...
                //     if ($(condition).value == 'null' || $(condition).value == 'notnull') {
                //         jQuery(parent).find(".value-attribute").each(function() {jQuery(this).css('visibility','hidden')});
                //     } else {
                //         jQuery(parent).find(".value-attribute").each(function() {jQuery(this).css('visibility','visible')});
                //     }
                // }
                // ,
                addRule: function (parent, elt) {
                    //console.log(parent);
                    console.log(elt);
                    elt.insert({after: ruleTemplate});
            ...
            script: {
                ...
                open:
                    ...
                    function (row, parent) {
                        ...
                        $$("#scripting #codemirror2")[0].setValue(value);
                        this.editor = parent.createCodeMirror($$("#scripting #codemirror2")[0], "application/x-httpd-php-open");
