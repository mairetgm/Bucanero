...
class Mapping extends \Magento\Backend\Block\Widget\Form\Generic implements \Magento\Backend\Block\Widget\Tab\TabInterface
{
    ...
    {
        $storeviewArray = !isset($data['storeviews']) || !count($data['storeviews']) ? array(0) : ($storeviewArray = $data['storeviews']);
        ...
        $mapping = $this->dataHelper->getJsonAttributes($this->dataHelper::MODULES_MAPPING);
        $storeviews = "<ul>";
        if (isset($mapping["storeviews"]["children"])) {
